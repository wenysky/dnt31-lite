...
namespace Discuz.Web
{
    ...
    public class topicadmin : PageBase
    {
        ...
        private bool DoMoveOperation()
        {
            ...
            TopicAdmins.MoveTopics(topiclist, moveto, forumid, type.CompareTo("redirect") == 0);
            return true;
        }
        ...
        private bool DoDelpostsOperation(string reason, ForumInfo forum, ref int layer)
        {
            ...
            foreach (string postid in postidlist.Split(','))
            {
                ...
                else
                {
                    ...
                    Posts.RemoveShowTopicCache(topic.Tid.ToString());
                    if (reval > 0 && config.Modworkstatus == 1)
                    {
                        string newTitle = post.Message.Replace(" ", "").Replace("|", "");
