...
namespace Discuz.Web
{
    ...
    public class modcp : PageBase
    {
        ...
        public int mpp = 16, pageid = 1, pagecount = 0, counts = 0;
        ...
        private void Login()
        {
            ...
            if (needshowlogin)
            {
                ...
                if (config.Passwordmode > 1)
                {
                    ...
                    {
                        userInfo = null;
                        AddErrLine(string.Format("用户名或密码错误"));
                        return;
                    }
                    uid = userInfo.Uid;
                }
                ...
                if (uid == -1 || userInfo.Adminid > 3 || userInfo.Adminid < 1)
                {
                    AddErrLine(string.Format("您当前的身份没有管理权限"));
                    return;
                }
        ...
        public void UpdateUser()
        {
            ...
            {
                op = "updateuser"; 
                SetUrl("modcp.aspx?operation=edituser&op=edit");
        ...
        public void UpdateForum()
        {
            ...
            if (useradminid != 3 || foruminfo.Alloweditrules != 0)
                foruminfo.Rules = DNTRequest.GetString("rulesnew");

            foruminfo.Description = DNTRequest.GetString("descnew");
            AdminForums.UpdateForumInfo(foruminfo);
            Context.Response.Redirect(BaseConfigs.GetForumPath + "modcp.aspx?operation=editforum&forumid=" + forumid);
        }
        ...
        private void UpdateUserCredits(string tidlist)
        {
            ...
            foreach (string tid in tidlist.Split(','))//如果该方法不需要使用于批量操作，可去掉foreach
            {
                ...
                if (values != null) //使用版块内积分
                    UserCredits.UpdateUserExtCredits(topic.Posterid, values);
                else //使用默认积分
        ...
        private void UpdateUserCredits(int postTableId, string pidlist)
        {
            ...
            foreach (string pid in pidlist.Split(','))
            {
                ...
                if (values != null) //使用版块内积分
                    UserCredits.UpdateUserExtCredits(post.Posterid, values);
                else //使用默认积分
        ...
        public void GetPostList()
        {
            ...
            pagenumbers = Utils.GetPageNumbers(pageid, pagecount, string.Format("modcp.aspx?operation=auditpost&forumid={0}&filter={1}", forumid, filter), 8);
        }
        ...
        public void AuditPost()
        {
            if (!Utils.StrIsNullOrEmpty(DNTRequest.GetString("pidlist")) && ismoder)
            {
                ...
                //    }
                //}
                for (int i = 0; i < pidlist.Length; i++)//foreach改for是doopcl改了，有什么问题联系我。
                {
                ...
                Posts.PassPost(tableid, validate, delete, ignore, DNTRequest.GetString("fidlist"));
                foreach (KeyValuePair<string, string> log in logs)
                {
                    AdminVistLogs.InsertLog(this.uid, this.username, this.usergroupid, GetGroupTile(), DNTRequest.GetIP(), log.Value.ToString(), log.Key.ToString());
                }
