...
namespace Discuz.Forum
{
    ...
    public class Attachments
    {
        ...
        public static int BindAttachment(AttachmentInfo[] attachmentinfo, int postid, StringBuilder msg, int topicid, int userid, UserGroupInfo usergroupinfo)
        {
            int acount = attachmentinfo.Length;
            // 附件查看权限
            ...
            for (int i = 0; i < acount; i++)
            {
                if (attachmentinfo[i] != null)
                {
                    ...
                    attachmentinfo[i].Attachprice = (attachprice != null && attachprice.Length > acount) ? UserGroups.CheckUserGroupMaxPrice(usergroupinfo, Utils.StrToInt(attachprice[i + 1], 0)) : 0;
        ...
        private static string GetAttachReplacement(PostpramsInfo postPramsInfo, int allowGetAttach, ShowtopicPageAttachmentInfo attachInfo)
        {
            ...
            else if (attachInfo.Allowread == 1)
            {
                ...
                else
                {
                    ...
                    if (postPramsInfo.CurrentUserGroup.Radminid == 1 || postPramsInfo.CurrentUserid == attachInfo.Uid || attachInfo.Attachprice <= 0 || attachInfo.Isbought == 1)
                        replacement = string.Format("<p><img alt=\"\" src=\"{0}images/attachicons/{6}\" border=\"0\"/><span class=\"bold\">附件</span>: <a href=\"{0}attachment.aspx?attachmentid={1}\" target=\"_blank\">{2}</a> ({3}, {4})<br />该附件被下载次数 {5}</p>", forumPath, attachInfo.Aid, attachInfo.Attachment.Trim(), attachInfo.Postdatetime, fileSize, attachInfo.Downloads, attachIcon);
                    else
