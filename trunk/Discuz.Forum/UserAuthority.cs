...
namespace Discuz.Forum
{
    ...
    public class UserAuthority
    {
        ...
        /// <returns></returns>
        public static int GetPostInvisible(ForumInfo forum, int useradminid, int disablepost)
        {
            int invisible = 1;
            if (forum.Modnewposts == 1 && useradminid != 1)
            {
                if (useradminid > 1)
                    invisible = disablepost == 1 ? 0 : 1;
                else
                    invisible = 1;
            }
            else
            ...
            if (useradminid != 1 && Scoresets.BetweenTime(GeneralConfigs.GetConfig().Postmodperiods))
                invisible = 0;
        ...
        public static bool NeedAudit(ForumInfo forum, int useradminid, int userid)
        {
            ...
            {
                if (forum.Modnewposts == 1 && !Moderators.IsModer(useradminid, userid, forum.Fid))
                {
                    needaudit = true;
                }
