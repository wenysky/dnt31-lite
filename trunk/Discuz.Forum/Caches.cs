...
namespace Discuz.Forum
{
    ...
    public class Caches
    {
        ...
        public static string GetTemplateListBoxOptionsCache()
        {
            lock (lockHelper)
            {
                ...
                if (Utils.StrIsNullOrEmpty(str))
                {
                    ...
                    str = sb.ToString();
                    cache.AddObject("/Forum/UI/TemplateListBoxOptions", str);
                    dt.Dispose();
                }
                return str;
            }
        ...
        {
            List<IpInfo> list = Discuz.Cache.DNTCache.GetCacheService().RetrieveObject("/Forum/BannedIp") as List<IpInfo>;

            if (list == null)
            {
                list = Ips.GetBannedIpList();
                Discuz.Cache.DNTCache.GetCacheService().AddObject("/Forum/BannedIp", list);
            }
            return list;
        }
