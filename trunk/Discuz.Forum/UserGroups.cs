...
namespace Discuz.Forum
{
    ...
    public class UserGroups
    {
        ...
        public static UserGroupInfo GetUserGroupInfo(int groupid)
        {
            ...
            {
                if (userGroupInfoList[i].Groupid == groupid)
        ...
        public static DataTable GroupParticipateScore(int uid, int gid)
        {
            ...
            if (dt != null)
            {
                ...
                {
                    dr = dt.Rows[count];
                    dr["MaxInDay"] = TypeConverter.ObjectToInt(dr["MaxInDay"]) - extcredits[TypeConverter.ObjectToInt(dr["ScoreCode"])];
                    if (!Convert.ToBoolean(dr["available"]) || TypeConverter.ObjectToInt(dr["MaxInDay"]) <= 0)
                    {
                        dr.Delete();
                        continue;
                    }

                    offset = TypeConverter.ObjectToInt(Math.Abs(Math.Ceiling((TypeConverter.ObjectToInt(dr["Max"]) - TypeConverter.ObjectToInt(dr["Min"])) / 10.0)));
                    sb.Remove(0, sb.Length);

                    for (int i = TypeConverter.ObjectToInt(dr["Max"]); i >= TypeConverter.ObjectToFloat(dr["Min"]); i -= offset)
                    {
                        if (i == 0)
                            continue;
                        if (Math.Abs(i) <= TypeConverter.ObjectToInt(dr["MaxInDay"]))
                            sb.AppendFormat("\n<li>{1}{2}</li>", i, i > 0 ? "+" : "", i);
                    }
