...
namespace Discuz.Data
{
    ...
    {    
        ...
        {
            return DatabaseProvider.GetInstance().CreateAttachment(attachmentinfo);
        }
        ...
        private static void DeleteAttachment(string aidlist, int pid, int tid)
        {
            ...
            if (tid > 0)
            {
                if (DatabaseProvider.GetInstance().GetAttachmentCountByPid(pid) <= 0)
                    DatabaseProvider.GetInstance().UpdatePostAttachment(pid, PostTables.GetPostTableId(tid), 0);
