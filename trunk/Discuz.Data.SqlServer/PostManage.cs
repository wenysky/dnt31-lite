...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public void PassAuditNewTopic(string tableId, string ignore, string validate, string delete, string fidList)
        {
            ...
            {
                DbHelper.ExecuteNonQuery(string.Format("UPDATE  [{0}topics]  SET [displayorder]=0 WHERE [tid] IN ({1}) AND [fid] IN ({2})",
                                                         BaseConfigs.GetTablePrefix,
                                                         validate,
                                                         fidList));
                DbHelper.ExecuteNonQuery(string.Format("UPDATE  [{0}posts{1}]  SET [invisible]=0 WHERE [layer]=0  AND ([tid] IN({2})) AND ([fid] IN ({3}))",
                                                         BaseConfigs.GetTablePrefix,
                                                         tableId,
                                                         validate,
                                                         fidList));
            ...
            {
                DbHelper.ExecuteNonQuery(string.Format("DELETE  FROM  [{0}topics]  WHERE [tid] IN ({1})  AND  [fid] IN ({2})",
                                                        BaseConfigs.GetTablePrefix,
                                                        delete,
                                                        fidList));
                DbHelper.ExecuteNonQuery(string.Format("DELETE  FROM  [{0}posts{1}]  WHERE [layer]=0  AND [tid] IN ({2})  AND [fid] IN ({3})",
                                                        BaseConfigs.GetTablePrefix,
                                                        tableId,
                                                        delete,
                                                        fidList));
            ...
            if (!Utils.StrIsNullOrEmpty(ignore))
                DbHelper.ExecuteNonQuery(string.Format("UPDATE  [{0}topics]  SET [displayorder]=-3 WHERE [tid] IN ({1})  AND [fid] IN ({2})",
                                         BaseConfigs.GetTablePrefix,
                                         ignore,
                                         fidList));
        }
        ...
        public int CreatePoll(PollInfo pollInfo)
        {
            ...
            string commandText = string.Format("INSERT INTO [{0}polls] ( [tid] ,[displayorder] ,[multiple] ,[visible] ,[maxchoices] ,[expiration] ,[uid] ,[voternames] ) VALUES (@tid, @displayorder, @multiple, @visible, @maxchoices, @expiration, @uid, @voternames);SELECT SCOPE_IDENTITY()  AS 'pollid'",
                                                BaseConfigs.GetTablePrefix);
        ...
        public bool UpdatePoll(PollInfo pollInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}polls] set [tid] = @tid, [displayorder] = @displayorder, [multiple] = @multiple, [visible] = @visible, [maxchoices] = @maxchoices, [expiration] = @expiration, [uid] = @uid, [voternames] = @voternames WHERE [pollid] = @pollid",
                                                BaseConfigs.GetTablePrefix);
        ...
        /// <returns></returns>
        public DataTable GetPostRateLogs(int pid, int displayRateCount)
        {
            string commandText = string.Format("SELECT TOP {0} {1} FROM [{2}ratelog] WHERE [pid]=@pid ORDER BY [id] DESC",
                                                displayRateCount,
                                                DbFields.RATE_LOG,
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteDataset(CommandType.Text, commandText, DbHelper.MakeInParam("@pid", (DbType)SqlDbType.Int, 4, pid)).Tables[0];
        }
        ...
        {
            string commandText = string.Format("UPDATE [{0}topics] SET [fid]=@fid, [typeid]=0 WHERE [tid] IN ({1})",
                                                BaseConfigs.GetTablePrefix,
                                                topicList);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText,
        ...
        public int RepairTopics(string topicList, string postTable)
        {
            ...
            if (iDataReader.Read())
            {
                commandText = string.Format("UPDATE [{0}topics] SET [lastpost] = '{1}' ,[lastpostid] = {2}, [lastposter] = '{3}', [lastposterid] = {4}, [replies] = (SELECT COUNT([pid]) FROM [{5}] WHERE [{0}topics].[tid] = [{5}].[tid]) - 1  WHERE [{0}topics].[tid] IN ({6})",
                                             BaseConfigs.GetTablePrefix,
