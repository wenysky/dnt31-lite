...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public DataTable GetTopicListByCondition(string postName, int forumId, string posterList, string keyList, string startDate, string endDate, int pageSize, int currentPage)
        {
            ...
            if (currentPage == 1)
            {
                commandText = string.Format("SELECT TOP {0} {1},f.[name] FROM [{2}topics] t JOIN " + 
                    ...
                    "OR CHARINDEX(',7,',','+CONVERT(NVARCHAR(1000),ff.[viewperm])+',')<>0) WHERE f.[fid]={3} " +
                    "OR CHARINDEX(',{3},',','+RTRIM(f.[parentidlist])+',')<>0 AND [password]='' AND [status]=1) AS f ON t.[fid]=f.[fid] " + 
            ...
            else
            {
                commandText = string.Format("SELECT TOP {0} {1},f.[name] FROM [{2}topics] t JOIN" + 
                    ...
                    "WHERE f.[fid]={3} OR CHARINDEX(',{3},',','+RTRIM(f.[parentidlist])+',')<>0 AND [password]='' AND [status]=1) AS f ON t.[fid]=f.[fid] " +
        ...
        {
            string commandText = string.Format("SELECT TOP {0} f.[fid], f.[name], f.[topics],f.[posts],ff.[rewritename] FROM [{1}forums] f LEFT JOIN [{1}forumfields] ff ON f.[fid]=ff.[fid] WHERE f.[status]=1 AND f.[layer]> 0 AND ff.[password]='' ORDER BY f.[posts] DESC, f.[todayposts] DESC", 
                                                topNumber, 
                                                BaseConfigs.GetTablePrefix);
