...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        {
            string commandText = string.Format("UPDATE [{0}users] SET [groupid]=4, [adminid]=0  WHERE [uid] IN ({1})", 
                                                BaseConfigs.GetTablePrefix, 
                                                uidList);
            DbHelper.ExecuteNonQuery(CommandType.Text, commandText);
        }
        ...
        public void UpdateOnlineList(UserGroupInfo userGroupInfo)
        {
            ...
            DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}updateonlinelist", BaseConfigs.GetTablePrefix), parms);
        }
        ...
        public int DeleteRowsByIP(string ip)
        {
            ...
            string commandText = string.Format("UPDATE [{0}users] SET [onlinestate]=0,[lastactivity]=GETDATE() WHERE [uid] IN (SELECT [userid] FROM [{0}online] WHERE [userid]>0 AND [ip]=@ip)", 
                                                BaseConfigs.GetTablePrefix);
            DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
            if (ip != "0.0.0.0")
