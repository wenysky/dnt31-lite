...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        {
            string commandText = string.Format("SELECT {0} FROM [{1}advertisements] ORDER BY [available] DESC",
        ...
        private string GetSearchByPosterSQL(bool spaceEnabled, bool albumEnabled, int posterId, int postTableId)
        {
            if (posterId > 0)
            {
                ...
                sqlBuilder.AppendFormat(@"SELECT DISTINCT [tid], 'forum' AS [datafrom] FROM [{0}posts{1}] WHERE [posterid]={2} AND [tid] NOT IN (SELECT [tid] FROM [{0}topics] WHERE [posterid]={2} AND [displayorder]<0)",
                                          ...
                                          posterId);
                if (spaceEnabled)
                    sqlBuilder.AppendFormat(@" UNION ALL SELECT [postid],'spacepost' AS [datafrom] FROM [{0}spaceposts] WHERE [uid]={1} ",
                                               BaseConfigs.GetTablePrefix,
                                               posterId);

                if (albumEnabled)
                    sqlBuilder.AppendFormat(@" UNION ALL SELECT [albumid],'album' AS [datafrom] FROM [{0}albums] WHERE [userid]={1}",
                                               BaseConfigs.GetTablePrefix,
                                               posterId);
        ...
        public int Search(bool spaceEnabled, bool albumEnabled, int postTableId, int userId, int userGroupId, string keyWord, int posterId, string type, string searchForumId, int keyWordType, int searchTime, int searchTimeType, int resultOrder, int resultOrderType)
        {
            ...
            {
                case 0:
                    searchType = (type == "digest") ? SearchType.DigestTopic : SearchType.PostTitle;
                    break;
                case 1:
