...
namespace Discuz.Web
{
    ...
    public class posttopic : PageBase
    {
        ...
        public PostInfo CreatePost(TopicInfo topicinfo)
        {
            ...
            postinfo.Title = useradminid == 1 ? Utils.HtmlEncode(DNTRequest.GetString(config.Antispamposttitle)) :
        ...
        private void NormalValidate(AdminGroupInfo admininfo, string postmessage, ShortUserInfo user)
        {
            ...
            if (Utils.StrIsNullOrEmpty(DNTRequest.GetString(config.Antispamposttitle)))
                AddErrLine("标题不能为空");
            else if (DNTRequest.GetString(config.Antispamposttitle).IndexOf("　") != -1)
