...
namespace Discuz.Web
{
    ...
    public class useradmin : PageBase 
    {
            ...
            if (!ispost)
            {
                ...
                if (action == "banuser")
                {
                    ...
                    switch (operateduser.Groupid)
                    {
                        case 4:
                            bantype = 1;
                            groupexpiry = "(" + FormatDate(operateduser.Groupexpiry.ToString()) + ")";
                            break;
                        case 5: 
                            bantype = 2;
                            groupexpiry = "(" + FormatDate(operateduser.Groupexpiry.ToString()) + ")";
        ...
        private string FormatDate(string orgDate)
        {
            if(orgDate.Length != 8)
                return orgDate;
            return orgDate.Substring(0, 4) + "-" + orgDate.Substring(4, 2) + "-" + orgDate.Substring(6);
        }
    }
}