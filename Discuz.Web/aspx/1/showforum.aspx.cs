...
namespace Discuz.Web
{
    ...
    public class showforum : PageBase
    {
        ...
        /// 主题分类Id
        /// </summary>
        public int topictypeid = DNTRequest.GetInt("typeid", 0);
        /// <summary>
        /// 过滤主题类型
        /// </summary>
        ...
        protected override void ShowPage()
        {
            ...
            //判断是否有快速发主题的权限
            if (canposttopic == true && (config.Fastpost == 1 || config.Fastpost == 3))
                    canquickpost = true;
            #endregion
        ...
        private void SetVisitedForumsCookie()
        {
            if (forum.Layer > 0)
            {
                ...
                if (visitedForums == "")
                {
                    Utils.WriteCookie("visitedforums", forum.Fid.ToString(), 43200);
                    return;
                }
                //版块存在于列表中
                if (("," + visitedForums + ",").IndexOf("," + forum.Fid + ",") != -1)
                    return;
                //访问过的版块未超过10个
                if (visitedForums.Split(',').Length < 10)
                {
                    Utils.WriteCookie("visitedforums", string.Format("{0},{1}", forum.Fid, visitedForums), 43200);
                    return;
                }
                //超过10，将最先访问过的去掉
                Utils.WriteCookie("visitedforums", string.Format("{0},{1}", forum.Fid, visitedForums.Substring(0, visitedForums.LastIndexOf(","))), 43200);
            }
        }
    }
}
