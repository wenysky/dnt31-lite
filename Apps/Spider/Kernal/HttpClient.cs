...
namespace Kernal
{
    public sealed class HttpClient : WebClient
    {
        // Cookie 容器
        ...
        public string GetSrc(string uriString, string dataEncoding, out string msg)
        {
            try
            {
                ...
                {
                    if (_proxy != "127.0.0.1" && _port != 0)
                    {
                        this._webproxy = new WebProxy(_proxy, _port);
                        this.Proxy = _webproxy;
                    }
                }
                ...
                srcString = srcString.Replace("\n", "");
                // 替换空格一些字符
                srcString = RegExpHelper.Replace(srcString, @">(\s*)<", "><");
                msg = srcString;
                return srcString;
            }
            catch (WebException we)
