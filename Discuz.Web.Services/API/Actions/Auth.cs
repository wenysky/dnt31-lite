...
namespace Discuz.Web.Services.API.Actions
{
    public class Auth : ActionBase
    {
        ...
        public string Register()
        {
            ...
            {
                ErrorCode = (int)ErrorType.API_EC_REGISTER_NOT_ALLOW;
                return returnStr;
            }
            ...
            //用户名不符合规范或者用户名已存在
            if (!CheckUsername(username) || Discuz.Forum.Users.GetUserId(username) != 0)
            {
                ErrorCode = (int)ErrorType.API_EC_REGISTER_NOT_ALLOW;
                return returnStr;
            }
