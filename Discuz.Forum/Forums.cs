...
namespace Discuz.Forum
{
    ...
    public class Forums
    {
        ...
        {
            DataTable dt = Discuz.Data.Forums.GetArchiverForumIndexList();
        ...
        public static string GetCurrentTopicTypesLink(int fid, string topictypes, string fullpagename)
        {
            ...
            if (topictypelinks == null)
            {
                ...
                foreach (string topictype in topictypes.Split('|'))
                {
                    if (!Utils.StrIsNullOrEmpty(topictype.Trim()))
                    {
                        ...
                        //下拉类型
                            ...
                            dropbuilder.AppendFormat("<p><a href=\"{0}?forumid={1}&typeid={2}\">{3}</a></p>", fullpagename, fid, topictype.Split(',')[0], topictype.Split(',')[1]);
                    }
                }
                if (!Utils.StrIsNullOrEmpty(dropbuilder.ToString()))
                {
                    ...
                    builder.AppendFormat("<div class=\"popupmenu_topictypeoption\">{0}</div>", dropbuilder.ToString().Trim());
                    builder.Append("</div>");
                }
        ...
        {
            Discuz.Common.Generic.List<IndexPageForumInfo> coll = new Discuz.Common.Generic.List<IndexPageForumInfo>();
            Discuz.Common.Generic.List<IndexPageForumInfo> returncoll = new Discuz.Common.Generic.List<IndexPageForumInfo>();
        ...
        public static string GetDropdownOptions()
        {
            ...
            if (result == null)
            {
                ...
                {
                    stringbuilder.AppendFormat("<option value=\"{0}\">{1}</option>", dr[0].ToString().Trim(), dr[1].ToString().Trim());
