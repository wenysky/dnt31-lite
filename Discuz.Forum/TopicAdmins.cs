...
namespace Discuz.Forum
{
    ...
    public class TopicAdmins
    {
        ...
        public static int SetDigest(string topiclist, int intValue)
        {
            ...
            if (mount > 0)
            {
                ...
                if (intValue > 0 && !useridlist.Equals(""))
                    UserCredits.UpdateUserCreditsAndExtCredits(useridlist, mount);
            }
            return mount;
        }
        ...
        public static int DeleteTopics(string topicList, int subTractCredits, bool reserveAttach)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (TypeConverter.ObjectToInt(dr["digest"]) > 0)
                    UserCredits.UpdateUserCreditsByDigest(TypeConverter.ObjectToInt(dr["posterid"]), -1);
            }
            ...
            if (dt != null)
            {
                ...
                ////int[] puidlist = new int[dt.Rows.Count];
                ...
                //        //    tuidlist[i] = TypeConverter.ObjectToInt(dr["posterid"]);
                //        //else
                //        //    auidlist[i] = Attachments.GetAttachmentCountByPid(TypeConverter.ObjectToInt(dr["pid"]));
                //        if (TypeConverter.ObjectToInt(dr["layer"]) == 0)
                //        {
                //            tuidlist[i] = TypeConverter.ObjectToInt(dr["posterid"]);
                //            //puidlist[i] = 0;
                ...
                //        {
                //            tuidlist[i] = 0;
                //            //puidlist[i] = TypeConverter.ObjectToInt(dr["posterid"]);
                ...
                UserCredits.UpdateUserCreditsByDeleteTopic((int[])tUidList.ToArray(typeof(int)), (int[])pUidList.ToArray(typeof(int)),att, -1);               
            }
        ...
        public static int MoveTopics(string topiclist, int fid, int oldfid)
        {
            ...
            //更新主题
            int reval = Discuz.Data.Topics.UpdateTopic(topiclist, fid);
            if (reval > 0)
            {
        ...
        public static void CancelRatePosts(string ratelogidlist, int tid, string pid, int userid, string username, int groupid, string grouptitle, int forumid, string forumname, string reason)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                SetPostRate(posttableid,
                             ...
                             TypeConverter.StrToFloat(dr["score"]),
                             false);
