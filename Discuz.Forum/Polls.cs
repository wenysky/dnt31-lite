...
namespace Discuz.Forum
{
    ...
    public class Polls
    {
        ...
        public static string GetVoters(int tid, int userid, string username, out bool allowvote)
        {
            ...
            for (int i = 0; i < votername.Length; i++)
            {
                ...
                if (votername[i].IndexOf(' ') == -1)
                    sbVoters.AppendFormat("<a href=\"userinfo.aspx?username={0}\">{1}</a>", Utils.UrlEncode(votername[i].Trim()), votername[i]);
                else
        ...
        /// <returns></returns>
        public static string CreatePoll(string pollItemName, int multiple, int maxchoices, int visiblepoll, string enddatetime, int tid, string[] pollitem, int userid)
        {
            string msg = null;
            ...
            if (PollItemname != "")
            {
                ...
                if (!Polls.CreatePoll(tid, multiple, pollitem.Length, PollItemname.Trim(), itemvaluelist.ToString().Trim(), enddatetime, userid, maxchoices, visiblepoll))
                    msg = "投票错误";
            }
            else
                msg = "投票项为空";
