...
namespace Discuz.Forum
{
    ...
    public class OnlineUsers
    {
        ...
        /// <summary>
        /// 缓存部分用户的临时信息，以减少对数据库的访问压力
        /// </summary>
        // private static Dictionary<string, OnlineUserInfo> onlineUserDic = new Dictionary<string, OnlineUserInfo>(500);//只保存500个用户信息
        ...
        {
            //string key = userid + ":" + password + ":" + DateTime.Now.ToString("yyyy-MM-dd HH:mm");//最后的时间确保用户信息在1分钟之后失效
            //if (onlineUserDic.ContainsKey(key))
            //{
            //    return onlineUserDic[key] as OnlineUserInfo;
            //}
            //else
            //{
            //    OnlineUserInfo onlineUserInfo = Discuz.Data.OnlineUsers.GetOnlineUser(userid, password);
            //    if (onlineUserDic.IsFull)//当达到存储上限时，则删除最先加入的用户信息
            //    {
            //        foreach (string keystr in onlineUserDic.Keys)
            //        {
            //            onlineUserDic.Remove(keystr);
            //            break;
            //        }
            //    }
            //    onlineUserDic.Add(key, onlineUserInfo);
