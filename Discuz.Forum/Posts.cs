...
namespace Discuz.Forum
{
    ...
    public class Posts
    {
        ...
        /// 缓存的帖子分页数
        /// </summary>
        private static int cachedPageNumber = 5;
        ...
        /// <summary>
        /// 移除主题帖子缓存,用于提升访问速度(注:当使用memcached时)
        /// </summary>
        /// <param name="tid"></param>
        public static void RemoveShowTopicCache(string tid)
        {
            MemCachedConfigInfo mcci = MemCachedConfigs.GetConfig();
            //只有在应用memcached的情况下才可以使用主题缓存
            if (mcci != null && mcci.ApplyMemCached)
            {
                for (int pageId = 1; pageId < cachedPageNumber; pageId++)
                {
                    Discuz.Cache.DNTCache.GetCacheService().RemoveObject("/Forum/ShowTopic_" + tid + "/Page_" + pageId);
                }
            }
        }
        ...
        public static List<PostInfo> GetUnauditPost(string fidList, int ppp, int pageIndex, int postTableId, int filter)
        {
            ...
            if (list != null)
            {
                ...
                {
                    ForumInfo forumInfo = Forums.GetForumInfo(postInfo.Pid);
                    if (forumInfo != null)
        ...
        public static int UpdatePostRateTimes(int tid, string postidlist)
        {
            ...
            RemoveShowTopicCache(tid.ToString());
        ...
        public static DataTable GetPostRateLogDataTable(int pid)
        {
            return pid > 0 ? Data.RateLogs.GetPostRateLogDataTable(pid) : null;
        }
        ...
        public static void PassPost(int postTableId, string validate, string delete, string ignore, string fidlist)
        {
            Data.Posts.PassPost(postTableId, validate, delete, ignore, fidlist);
        }
            ...
            foreach (string idlist in DNTRequest.GetString("pid").Split(','))
            {
                ...
                if (pid.Trim() != "")
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        RemoveShowTopicCache(dt.Rows[0]["tid"].ToString());
                    }
                }
            }
        }
